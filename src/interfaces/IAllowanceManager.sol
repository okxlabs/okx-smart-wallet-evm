...
interface IAllowanceManager is IERC7914 {
    ...
    function approveToken(address token, address spender, uint256 amount) external returns (bool success);
    ...
    /// @param amount The amount to approve
    /// @return success True if approval succeeded
    function approveTokenTransient(address token, address spender, uint256 amount) external returns (bool success);
    ...
    function transferFromToken(address token, address from, address recipient, uint256 amount) external returns (bool success);
    ...
    /// @param amount The amount to transfer
    /// @return success True if transfer succeeded
    function transferFromTokenTransient(address token, address from, address recipient, uint256 amount) external returns (bool success);
    
    /// @notice Get the current persistent token allowance
    /// @param token The ERC20 token address
    /// @param spender The spender address
    /// @return allowance The current allowance
    function tokenAllowance(address token, address spender) external view returns (uint256 allowance);
    
    /// @notice Get the current transient token allowance
    /// @param token The ERC20 token address
    /// @param spender The spender address
    /// @return allowance The current transient allowance
    function transientTokenAllowance(address token, address spender) external view returns (uint256 allowance);
}
