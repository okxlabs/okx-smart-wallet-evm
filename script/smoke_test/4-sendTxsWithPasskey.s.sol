...
contract SendTxsWithPasskey is Script {
    ...
    function _addEoaOwnerWithMerkle(
        ...
        console.log(
            "Executing addOwner with Passkey signature and Merkle proof via executeWithRelayer..."
        );
        ISmartWallet(userWallet).executeWithRelayer(batchedCall, validatorData);

        console.log(
            "Successfully added EOA owner via executeWithRelayer using Merkle proof!"
        );
    }
        ...
        )
    {
        ...
        batchedCall = BatchedCall({calls: calls, nonce: nonce});
