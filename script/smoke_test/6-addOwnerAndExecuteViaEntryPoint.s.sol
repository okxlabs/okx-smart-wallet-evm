...
contract AddOwnerAndExecuteViaEntryPoint is Script {
    ...
    function _addPasskeyOwnerViaEntryPoint(
        ...
        bytes32 newOwnerKeyHash = keccak256(
            abi.encodePacked(newOwnerPubX, newOwnerPubY)
        );
        console.log("Adding new Passkey owner with keyHash:");
        console.logBytes32(newOwnerKeyHash);

        // Create calls for adding owner
        Call[] memory calls = _createAddOwnerCall(
            userWallet,
            newOwnerKeyHash,
            validator
        );
        ...
        console.log(
            "Successfully added second Passkey as owner via EntryPoint using Merkle proof!"
        );
    }
        ...
        )
    {
        ...
        userOp = PackedUserOperation({
            ...
            accountGasLimits: bytes32(
                (uint256(2000000) << 128) | uint256(400000)
            ),
    ...
    function _executeWithPasskeyViaEntryPoint(
        ...
        console.log(
            "Transaction executed successfully via EntryPoint with Passkey!"
        );
    }
    ...
    function _createPasskeySignature(
        ...
        (string memory clientDataJson, , bytes32 passkeyMessageHash) = HelperLib
            .getPasskeyMessageHash(hashWithValidUntil);
