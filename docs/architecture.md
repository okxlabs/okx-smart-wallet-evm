...
OKX smart wallet technology has been designed to support multiple use cases and business scenarios:
...
As okx smart wallet service expands, we've designed an architecture to improve maintainability, flexibility, and reusability.
...
7. **Hook**: Per-owner extensible execution callbacks
  - Owner-specific hook registered via packed settings
  - Callbacks invoked before and after batch execution
  - Hook revert at either phase cancels the entire transaction
  - No hook for `address(this)` (built-in EIP-7702 EOA owner)
...
6. **Granular Access Control**: Per-owner admin flags, expiration timestamps, and hooks allow different owners to carry independent privileges and constraints
