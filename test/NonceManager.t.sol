...
contract NonceManagerTest is Test {
    ...
    // ============ Integration Tests with SmartWallet ============
    // These tests mirror common usage patterns found in other test files

    function test_nonce_usage_pattern_like_validation_tests() public view {
        // Pattern found in Validation.t.sol and other files
        uint192 defaultKey = uint192(0); // Most tests use key 0

        // Test the pattern: uint256(INonceManager(account).getNonce(uint192(0)))
        uint64 currentNonce = nonceManager.getNonce(defaultKey);
        assertEq(currentNonce, 0);
    }

    function test_nonce_key_extraction_pattern() public pure {
        // Pattern found in MerkleExecution.t.sol:331-333
        uint192 nonceKey = uint192(0);
        uint64 expectedNonce = 5;
        uint256 packedNonce = (uint256(nonceKey) << 64) |
            uint256(expectedNonce);

        // Verify the extraction works as expected in real usage
        uint192 extractedKey = uint192(packedNonce >> 64);
        uint64 extractedNonce = uint64(packedNonce);

        assertEq(extractedKey, nonceKey);
        assertEq(extractedNonce, expectedNonce);
    }

    function test_nonce_event_emission_pattern() public {
        // Pattern found in Validation.t.sol:117-144
        uint192 key = uint192(0); // Default key used in most tests
        uint256 expectedNonce = 0;
        uint256 packedNonce = (uint256(key) << 64) | uint256(expectedNonce);

        // Test event emission with default key
        vm.expectEmit(true, true, true, true);
        emit NonceConsumed(key, uint64(expectedNonce));

        bool result = nonceManager.testValidateAndUpdateNonce(packedNonce);
        assertTrue(result);
    }

    function test_invalid_nonce_error_scenario() public {
        // Pattern found in MerkleExecution.t.sol:327-350
        // Simulates the scenario where wrong nonce is provided
        uint192 nonceKey = uint192(0);
        uint64 wrongNonce = 1; // Expected is 0, but providing 1
        uint256 invalidPackedNonce = (uint256(nonceKey) << 64) |
            uint256(wrongNonce);

        // This should return false but still increment the nonce
        bool result = nonceManager.testValidateAndUpdateNonce(
            invalidPackedNonce
        );
        assertFalse(result);

        // Nonce should still be incremented to 1
        assertEq(nonceManager.getNonce(nonceKey), 1);
    }

    function test_replay_attack_prevention_pattern() public {
        // Pattern found in MerkleExecution.t.sol:358-380
        // Simulates replay attack prevention
        uint192 key = uint192(0);
        uint256 packedNonce = (uint256(key) << 64) | uint256(0);

        // First execution should succeed
        bool firstResult = nonceManager.testValidateAndUpdateNonce(packedNonce);
        assertTrue(firstResult);

        // Second execution with same nonce should fail (replay attack)
        bool replayResult = nonceManager.testValidateAndUpdateNonce(
            packedNonce
        );
        assertFalse(replayResult);

        // But the nonce should have incremented both times
        assertEq(nonceManager.getNonce(key), 2);
    }
