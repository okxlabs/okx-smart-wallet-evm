...
contract TransferWithAuthorizationIntegrationTest is TwaIntegrationBase {
    ...
    function test_multiActor_perKeyHookIsolationAcrossOwners() public {
        ...
        uint256 settings = _packSettings(false, 0, address(bobHook));
    ...
    function test_recovery_hookBlockThenRemoveHookThenRetrySameSignature() public {
        ...
        bytes32 bobKeyHash = keccak256(abi.encodePacked(_bob));
        uint256 blockedSettings = _packSettings(false, 0, address(blockingHook));
        ...
        uint256 openSettings = _packSettings(false, 0, address(0));
        Call[] memory calls = new Call[](1);
        calls[0] = Call({
            target: _aliceWallet,
            value: 0,
