...
contract AllowanceManagerTest is Base {
    ...
    function test_ApproveToken_Success() public {
        ...
        vm.prank(_alice);
        bool success = WalletCore(_alice).approveToken(address(mockToken), spender, amount);
        
        assertTrue(success);
        assertEq(WalletCore(_alice).tokenAllowance(address(mockToken), spender), amount);
    }
    ...
    function test_ApproveToken_OnlyOwnerOrEntryPoint() public {
        ...
        vm.expectRevert();
        WalletCore(_alice).approveToken(address(mockToken), spender, amount);
        ...
        vm.prank(_alice);
        WalletCore(_alice).approveToken(address(mockToken), spender, amount * 2);
        assertEq(WalletCore(_alice).tokenAllowance(address(mockToken), spender), amount * 2);
    }
    ...
    function test_ApproveTokenTransient_Success() public {
        ...
        vm.prank(_alice);
        bool success = WalletCore(_alice).approveTokenTransient(address(mockToken), spender, amount);
        
        assertTrue(success);
        assertEq(WalletCore(_alice).transientTokenAllowance(address(mockToken), spender), amount);
    }
    ...
    function test_TransferFromToken_Success() public {
        ...
        // Set up allowance
        vm.prank(_alice);
        WalletCore(_alice).approveToken(address(mockToken), spender, allowanceAmount);
        ...
        vm.prank(spender);
        bool success = WalletCore(_alice).transferFromToken(
            address(mockToken), 
            _alice, 
            recipient, 
            transferAmount
        ...
        assertEq(WalletCore(_alice).tokenAllowance(address(mockToken), spender), allowanceAmount - transferAmount);
    }
    ...
    function test_TransferFromToken_InsufficientAllowance() public {
        ...
        WalletCore(_alice).transferFromToken(
            address(mockToken), 
            _alice, 
            recipient, 
            transferAmount
    ...
    function test_TransferFromToken_IncorrectSender() public {
        vm.prank(_alice);
        WalletCore(_alice).approveToken(address(mockToken), spender, 100 * 10**18);
    ...
    function test_TransferFromTokenTransient_Success() public {
        ...
        vm.prank(_alice);
        WalletCore(_alice).approveTokenTransient(address(mockToken), spender, allowanceAmount);
        ...
        vm.prank(spender);
        bool success = WalletCore(_alice).transferFromTokenTransient(
            address(mockToken), 
            _alice, 
            recipient, 
            transferAmount
        ...
        assertTrue(success);
        assertEq(mockToken.balanceOf(recipient), initialBalance + transferAmount);
        assertEq(WalletCore(_alice).transientTokenAllowance(address(mockToken), spender), allowanceAmount - transferAmount);
    }
    ...
    function test_TransferFromToken_UnlimitedAllowance() public {
        ...
        vm.prank(_alice);
        WalletCore(_alice).approveToken(address(mockToken), spender, type(uint256).max);
        ...
        assertEq(WalletCore(_alice).tokenAllowance(address(mockToken), spender), type(uint256).max);
    }
    ...
    function test_ApproveNative_Success() public {
        ...
        vm.expectEmit(true, true, false, true);
        emit ApproveNative(address(WalletCore(_alice)), spender, amount);
        
        vm.prank(_alice);
        bool success = WalletCore(_alice).approveNative(spender, amount);
        
        assertTrue(success);
        assertEq(WalletCore(_alice).nativeAllowance(spender), amount);
    }
    ...
    function test_ApproveNative_OnlyOwnerOrEntryPoint() public {
        ...
        vm.prank(unauthorized);
        vm.expectRevert();
        WalletCore(_alice).approveNative(spender, amount);
        ...
        vm.prank(address(WalletCore(_alice)));
        WalletCore(_alice).approveNative(spender, amount * 2);
        assertEq(WalletCore(_alice).nativeAllowance(spender), amount * 2);
    }
    ...
    function test_ApproveNativeTransient_Success() public {
        ...
        vm.expectEmit(true, true, false, true);
        emit ApproveNativeTransient(address(WalletCore(_alice)), spender, amount);
        
        vm.prank(_alice);
        bool success = WalletCore(_alice).approveNativeTransient(spender, amount);
        
        assertTrue(success);
        assertEq(WalletCore(_alice).transientNativeAllowance(spender), amount);
    }
    ...
    function test_TransferFromNative_Success() public {
        ...
        // Set up allowance
        vm.prank(_alice);
        WalletCore(_alice).approveNative(spender, allowanceAmount);
        ...
        vm.expectEmit(true, true, false, true);
        emit TransferFromNative(address(WalletCore(_alice)), recipient, transferAmount);
        
        vm.prank(spender);
        bool success = WalletCore(_alice).transferFromNative(
            _alice, 
            recipient, 
            transferAmount
        );
        ...
        assertEq(WalletCore(_alice).nativeAllowance(spender), allowanceAmount - transferAmount);
    }
    ...
    function test_TransferFromNative_InsufficientAllowance() public {
        ...
        WalletCore(_alice).transferFromNative(
            _alice, 
            recipient, 
            transferAmount
        );
    ...
    function test_TransferFromNativeTransient_Success() public {
        ...
        // Set up transient allowance
        vm.prank(_alice);
        WalletCore(_alice).approveNativeTransient(spender, allowanceAmount);
        
        uint256 initialBalance = recipient.balance;
        
        vm.expectEmit(true, true, false, true);
        emit TransferFromNativeTransient(address(WalletCore(_alice)), recipient, transferAmount);
        
        vm.prank(spender);
        bool success = WalletCore(_alice).transferFromNativeTransient(
            _alice, 
            recipient, 
            transferAmount
        );
        ...
        assertTrue(success);
        assertEq(recipient.balance, initialBalance + transferAmount);
        assertEq(WalletCore(_alice).transientNativeAllowance(spender), allowanceAmount - transferAmount);
    }
    ...
    function test_MultipleSpenders() public {
        ...
        vm.prank(_alice);
        WalletCore(_alice).approveToken(address(mockToken), spender, amount1);
        
        vm.prank(_alice);
        WalletCore(_alice).approveToken(address(mockToken), spender2, amount2);
        
        assertEq(WalletCore(_alice).tokenAllowance(address(mockToken), spender), amount1);
        assertEq(WalletCore(_alice).tokenAllowance(address(mockToken), spender2), amount2);
    }
    ...
    function test_MultipleTokens() public {
        ...
        vm.prank(_alice);
        WalletCore(_alice).approveToken(address(token2), spender, amount2);
        
        assertEq(WalletCore(_alice).tokenAllowance(address(mockToken), spender), amount1);
        assertEq(WalletCore(_alice).tokenAllowance(address(token2), spender), amount2);
    }
    ...
    function test_TransientStorageIsolation() public {
        ...
        vm.prank(_alice);
        WalletCore(_alice).approveTokenTransient(address(mockToken), spender, amount);
    ...
    function test_OverwriteAllowances() public {
        ...
        vm.prank(_alice);
        WalletCore(_alice).approveToken(address(mockToken), spender, initialAmount);
        
        // Overwrite with new amount
        vm.prank(_alice);
        WalletCore(_alice).approveToken(address(mockToken), spender, newAmount);
        
        assertEq(WalletCore(_alice).tokenAllowance(address(mockToken), spender), newAmount);
    }
