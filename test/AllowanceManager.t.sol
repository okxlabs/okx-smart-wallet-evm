...
contract AllowanceManagerTest is Base {
    ...
    function test_ApproveTokenTransient_Success() public {
        ...
        vm.prank(_alice);
        bool success = SmartWallet(_alice).approveTokenTransient(
            address(mockToken),
            spender,
            amount
        );
        ...
        assertTrue(success);
        assertEq(
            SmartWallet(_alice).transientTokenAllowance(
                address(mockToken),
                spender
            ),
            amount
    ...
    function test_TransferFromTokenTransient_Success() public {
        ...
        vm.prank(_alice);
        SmartWallet(_alice).approveTokenTransient(
            address(mockToken),
            spender,
            allowanceAmount
        );
        ...
        vm.expectEmit(true, true, false, true);
        emit TransferFromTokenTransient(
            address(mockToken),
            recipient,
            transferAmount
        );
    ...
    function test_ApproveNativeTransient_Success() public {
        ...
        vm.prank(_alice);
        bool success = SmartWallet(_alice).approveNativeTransient(
            spender,
            amount
        );
    ...
    function test_TransferFromNativeTransient_Success() public {
        ...
        vm.expectEmit(true, true, false, true);
        emit TransferFromNativeTransient(
            address(SmartWallet(_alice)),
            recipient,
            transferAmount
        );
        ...
        assertEq(recipient.balance, initialBalance + transferAmount);
        assertEq(
            SmartWallet(_alice).transientNativeAllowance(spender),
            allowanceAmount - transferAmount
        );
    }
    ...
    function test_MultipleTokens() public {
        ...
        assertEq(
            SmartWallet(_alice).tokenAllowance(address(token2), spender),
            amount2
        );
    }
