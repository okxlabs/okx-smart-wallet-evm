...
contract CreateAccount is Script {
    function run() external {
        ...
        console.log("Creating new SmartWallet account with Passkey and ECDSA owners...");
        ...
        uint256 randomECDSAPrivateKey = uint256(keccak256(abi.encodePacked(block.timestamp, deployer, "RANDOM_ECDSA_OWNER")));
        ...
        uint256 salt = uint256(keccak256(abi.encodePacked(block.timestamp, deployer, "SMART_WALLET_V1")));
        console.log("Using salt: ", salt);
        
        // Create the account
        address newAccount = ISmartWalletFactory(factory).createAccount(initialOwners, salt);
    ...
    function createAccountWithParams(
        ...
        // Random ECDSA owner
        uint256 randomECDSAPrivateKey = uint256(keccak256(abi.encodePacked(customSalt, deployer, "RANDOM_ECDSA_OWNER")));
        address randomECDSAAddress = vm.addr(randomECDSAPrivateKey);
        bytes32 ecdsaKeyHash = bytes32(uint256(uint160(randomECDSAAddress)));
        initialOwners[1] = InitialOwner({
            keyHash: ecdsaKeyHash,
        ...
        // Create the account
        address newAccount = ISmartWalletFactory(factory).createAccount(initialOwners, customSalt);
