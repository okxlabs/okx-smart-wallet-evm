...
contract CreateAccountWithAddOwner is Script {
    ...
    function _executeAccountCreation(address deployer) private returns (address) {
        ...
        bytes32 passkeyKeyHash = keccak256(abi.encodePacked(passkeyPubX, passkeyPubY));
        ...
        uint256 salt = uint256(keccak256(abi.encodePacked(block.timestamp, deployer, "SMART_WALLET_WITH_CALL")));
        console.log("Using salt: ", salt);
        
        // 3. Predict the account address
        address predictedAddress = SmartWalletFactory(vm.envAddress("SMART_WALLET_FACTORY")).getAddress(initialOwners, salt);
        console.log("Predicted account address: ", predictedAddress);
        
        // 4. Generate random EOA owner to be added
        address randomEOAAddress = vm.addr(uint256(keccak256(abi.encodePacked(block.timestamp, deployer, "RANDOM_EOA_OWNER"))));
        ...
        return SmartWalletFactory(vm.envAddress("SMART_WALLET_FACTORY")).createAccountWithCall(
            initialOwners,
            salt,
            batchedCall,
            validatorData
    ...
    function _createAddOwnerCall(
        ...
        settings |= uint256(uint160(0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48)) << 96;
    ...
    function _computeTypedHashSansChainId(bytes32 structHash, address predictedWallet) private pure returns (bytes32) {
